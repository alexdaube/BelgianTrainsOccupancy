...
def main():
    ...
    #
    # # (rush_hour_morning = 1) ^ (to_urban =1) → (Occupancy = HIGH) / (rush_hour_morning = 1) ^ (to_urban =1) → (Occupancy = MEDIUM) / (rush_hour_morning = 1) ^ (to_urban =1) → (Occupancy = LOW)
    # am_entries.where(lambda row: 1 == row['in_morning_rush']).where(lambda row: 1 == row['to_urban']).pivot(
    #     'to_urban', 'occupancy').print_table()
    #
    # # (rush_hour_morning = 1) ^ (to_urban =1) ^ (from_urban = 1) et (rush_hour_morning = 1) ^ (to_urban =1) ^ (from_urban = 0)
    # am_entries.where(lambda row: 1 == row['in_morning_rush']).where(lambda row: 1 == row['to_urban']).pivot(
    #     'from_urban').print_table()
    #
    # # (rush_hour_morning = 1) ^ (to_urban =1) ^ (from_urban = 1)
    # am_entries.where(lambda row: 1 == row['in_morning_rush']).where(lambda row: 1 == row['to_urban']).where(lambda row: 0 == row['from_urban']).pivot(
    #     'from_urban', 'occupancy').print_table()
    #
    #
    # print("\nPM entries")
